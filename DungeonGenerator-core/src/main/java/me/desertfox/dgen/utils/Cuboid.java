...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
        ...
        public boolean isCollidingWith(Cuboid other) {
                if (this.world == null || other.world == null || !this.world.getUID().equals(other.world.getUID())) {
                        return false;
                }

                boolean xOverlap = this.x1 <= other.x2 && this.x2 >= other.x1;
                boolean yOverlap = this.y1 <= other.y2 && this.y2 >= other.y1;
                boolean zOverlap = this.z1 <= other.z2 && this.z2 >= other.z1;
        ...
        public Location getLocation1() {
                Objects.requireNonNull(world, "Cuboid's world is undefined");
        ...
        public Location getClosestPoint(Location point) {
                double x1 = Math.min(this.x1, x2);
                double y1 = Math.min(this.y1, y2);
                double z1 = Math.min(this.z1, z2);

                double x2 = Math.max(this.x1, this.x2);
                double y2 = Math.max(this.y1, this.y2);
                double z2 = Math.max(this.z1, this.z2);

                double closestX = clamp(point.getX(), x1, x2);
                double closestY = clamp(point.getY(), y1, y2);
                double closestZ = clamp(point.getZ(), z1, z2);

                return new Location(point.getWorld(), closestX, closestY, closestZ);
        }
        ...
        public Cuboid(Location l1, Location l2) {
                if (!l1.getWorld().getUID().equals(l2.getWorld().getUID())) throw new IllegalStateException("Locations must be on the same world");
        ...
        /**
         * Construct a Cuboid given two Vector objects which represent any two corners of the Cuboid.
         * @param v1 One of the corners
         ...
         */
        private Cuboid(@Nullable World world, int x1, int y1, int z1, int x2, int y2, int z2) {
                this.world = world;
                this.x1 = Math.min(x1, x2);
                this.x2 = Math.max(x1, x2);
                this.y1 = Math.min(y1, y2);
        ...
        public Cuboid(Map<String, Object> map) {
                this.world = Bukkit.getWorld((String) map.get("worldName"));
        ...
        public Location getCenter() {
                ...
                return new Location(this.world, this.getLowerX() + (x1 - this.getLowerX()) / 2.0, this.getLowerY() + (y1 - this.getLowerY()) / 2.0, this.getLowerZ() + (z1 - this.getLowerZ()) / 2.0);
        }
        ...
        public Cuboid expand(CuboidDirection dir, int amount) {
                return switch (dir) {
                        case North -> new Cuboid(this.world, this.x1 - amount, this.y1, this.z1, this.x2, this.y2, this.z2);
                        case South -> new Cuboid(this.world, this.x1, this.y1, this.z1, this.x2 + amount, this.y2, this.z2);
                        case East -> new Cuboid(this.world, this.x1, this.y1, this.z1 - amount, this.x2, this.y2, this.z2);
                        case West -> new Cuboid(this.world, this.x1, this.y1, this.z1, this.x2, this.y2, this.z2 + amount);
                        case Down -> new Cuboid(this.world, this.x1, this.y1 - amount, this.z1, this.x2, this.y2, this.z2);
                        case Up -> new Cuboid(this.world, this.x1, this.y1, this.z1, this.x2, this.y2 + amount, this.z2);
        ...
        public boolean contains(Location l) {
                if (world == null || !world.getUID().equals(l.getWorld().getUID())) return false;
        ...
        public Cuboid contract(CuboidDirection dir) {
                ...
                return switch (dir) {
                        case Down -> {
                            ...
                            yield new Cuboid(this.world, this.x1, this.y1, this.z1, this.x2, face.getUpperY(), this.z2);
                        }
                        case Up -> {
                            while (face.containsOnly(0) && face.getUpperY() < this.getUpperY()) {
                                face = face.shift(CuboidDirection.Up, 1);
                            }
                            yield new Cuboid(this.world, this.x1, face.getLowerY(), this.z1, this.x2, this.y2, this.z2);
                        }
                        case North -> {
                            while (face.containsOnly(0) && face.getLowerX() > this.getLowerX()) {
                                face = face.shift(CuboidDirection.North, 1);
                            }
                            yield new Cuboid(this.world, this.x1, this.y1, this.z1, face.getUpperX(), this.y2, this.z2);
                        }
                        case South -> {
                            while (face.containsOnly(0) && face.getUpperX() < this.getUpperX()) {
                                face = face.shift(CuboidDirection.South, 1);
                            }
                            yield new Cuboid(this.world, face.getLowerX(), this.y1, this.z1, this.x2, this.y2, this.z2);
                        }
                        case East -> {
                            while (face.containsOnly(0) && face.getLowerZ() > this.getLowerZ()) {
                                face = face.shift(CuboidDirection.East, 1);
                            }
                            yield new Cuboid(this.world, this.x1, this.y1, this.z1, this.x2, this.y2, face.getUpperZ());
                        }
                        case West -> {
                            while (face.containsOnly(0) && face.getUpperZ() < this.getUpperZ()) {
                                face = face.shift(CuboidDirection.West, 1);
                            }
                            yield new Cuboid(this.world, this.x1, this.y1, face.getLowerZ(), this.x2, this.y2, this.z2);
                        }
                        default -> throw new IllegalArgumentException("Invalid direction " + dir);
                    };
        }
        ...
        public Cuboid getFace(CuboidDirection dir) {
                return switch (dir) {
                        case Down -> new Cuboid(this.world, this.x1, this.y1, this.z1, this.x2, this.y1, this.z2);
                        case Up -> new Cuboid(this.world, this.x1, this.y2, this.z1, this.x2, this.y2, this.z2);
                        case North -> new Cuboid(this.world, this.x1, this.y1, this.z1, this.x1, this.y2, this.z2);
                        case South -> new Cuboid(this.world, this.x2, this.y1, this.z1, this.x2, this.y2, this.z2);
                        case East -> new Cuboid(this.world, this.x1, this.y1, this.z1, this.x2, this.y2, this.z1);
                        case West -> new Cuboid(this.world, this.x1, this.y1, this.z2, this.x2, this.y2, this.z2);
                        default -> throw new IllegalArgumentException("Invalid direction " + dir);
                    };
        }
        ...
        public Cuboid getBoundingCuboid(Cuboid other) {
                ...
                return new Cuboid(this.world, xMin, yMin, zMin, xMax, yMax, zMax);
        }

        /**
         * Added by DesertFoxHU
         * @return A random location between the cuboid's borders
         */
        public Location getRandomLocation() {
                Objects.requireNonNull(world, "Cuboid's world is undefined");
                int x = new Random().nextInt(getUpperX() + 1 - getLowerX()) + getLowerX();
                int y = new Random().nextInt(getUpperY() + 1 - getLowerY()) + getLowerY();
                int z = new Random().nextInt(getUpperZ() + 1 - getLowerZ()) + getLowerZ();
        ...
        public Block getRelativeBlock(int x, int y, int z) {
                Objects.requireNonNull(this.getWorld(), "Cuboid's world is undefined");
        ...
        public String toString() {
                return "Cuboid: " + (this.world != null ? this.world.getName() + "," : null) + this.x1 + "," + this.y1 + "," + this.z1 + "=>" + this.x2 + "," + this.y2 + "," + this.z2;
        }
